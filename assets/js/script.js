...
async function fetchGitHubData() {
    ...
    if (cachedData) {
        console.log('Usando dados do cache para skills...');
        
        // Atualizar perfil e repos normalmente (dados leves)
        try {
            ...
            if (reposResponse.ok) {
                const recentRepos = await reposResponse.json();
                if (Array.isArray(recentRepos)) {
                    updateGitHubRepos(recentRepos);
                }
            }
        } catch (error) {
            console.warn('Erro ao buscar dados básicos:', error);
        }
        
        // Usar dados cached para skills
        const { languageStats, technologyStats, totalBytes } = cachedData;
        generateSkillsCards(languageStats, technologyStats, totalBytes, true); // true = fromCache
        return;
    }
    
    try {
        // Tentar buscar dados do perfil com retry
        ...
        try {
            ...
            if (profileResponse.ok) {
                ...
                console.warn('Rate limit da API do GitHub atingido para perfil');
            }
        } catch (profileError) {
            console.warn('Erro ao buscar perfil:', profileError);
        }

        // Tentar buscar repositórios com retry
        let allRepos = [];
        try {
            ...
            if (reposResponse.ok) {
                ...
                if (Array.isArray(allRepos)) {
                    const recentRepos = allRepos.slice(0, 5);
                    console.log('Repositórios recentes encontrados:', recentRepos.length);
                    updateGitHubRepos(recentRepos);
                    
                    // Analisar habilidades técnicas de todos os repositórios
                    await analyzeSkillsFromRepos(username, allRepos);
                } else {
                    throw new Error('Invalid response from GitHub API');
                }
            } else if (reposResponse.status === 403) {
                console.warn('Rate limit da API do GitHub atingido para repositórios');
                throw new Error('GitHub API rate limit');
            } else {
                throw new Error(`GitHub API Error: ${reposResponse.status}`);
            }
        } catch (reposError) {
            console.warn('Erro ao buscar repositórios:', reposError);
            throw reposError; // Propagar para o catch principal
        }
        
    } catch (error) {
        console.error('Erro ao buscar dados do GitHub:', error);
        
        // Usar dados de fallback estáticos quando a API falha
        console.log('Usando repositórios de fallback estáticos...');
        const fallbackRepos = [
            {
            ...
            },
            {
                ...
                description: "Coleção de projetos em JavaScript",
                html_url: "https://github.com/mikaelfmts/javascript-projects",
                language: "JavaScript",
                stargazers_count: 0,
                forks_count: 0
            },
            {
                ...
                html_url: "https://github.com/mikaelfmts/web-dev",
                language: "HTML",
                stargazers_count: 0,
                forks_count: 0
            }
...
// Função para mostrar skills de fallback
function showFallbackSkills() {
    ...
    let skillsHTML = '';
    Object.entries(FALLBACK_SKILLS).forEach(([skillName, data]) => {
        const skillInfo = {
            category: data.type,
            icon: data.icon,
            color: data.color
        ...
        skillsHTML += createSkillCard(skillName, data.percentage, skillInfo, { type: 'fallback' });
    });
    
    skillsGrid.innerHTML = skillsHTML;
    
    // Adicionar informação sobre fallback
    const fallbackInfo = `
        <div class="analysis-info fallback" style="grid-column: 1 / -1; margin-top: 1rem; padding: 1rem; background: rgba(255, 193, 7, 0.1); border-radius: 8px; text-align: center; border-left: 4px solid #ffc107;">
            <p style="margin: 0; color: #ffc107; font-size: 0.9rem;">
                <i class="fas fa-exclamation-triangle" style="margin-right: 0.5rem;"></i>
                <strong>Modo Offline:</strong> Exibindo habilidades estáticas. A análise automática será executada quando a conexão com a API do GitHub for restaurada.
                <br>
                <small style="opacity: 0.8;">Recarregue a página para tentar novamente • Dados estáticos baseados no portfólio</small>
            </p>
        </div>
    `;
    
    skillsGrid.insertAdjacentHTML('beforeend', fallbackInfo);
    
    console.log('Skills de fallback carregadas devido a erro na API do GitHub');
}
