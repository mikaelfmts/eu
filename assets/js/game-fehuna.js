...
class FehunaGame {
    constructor(canvasId) {
        ...
        // Constantes visuais do estilo LoL
        this.LOL_COLORS = {
            gold: '#c8aa6e',
            goldLight: '#f0e6d2',
            goldDark: '#785a28',
        ...
        // Carregamento de recursos visuais do LoL
        this.lolAssets = {
            fonts: ['Beaufort for LOL', 'Spiegel'],
            loaded: false,
            runeterraMaps: {}
        };
    ...
    }    // Renderização com estilo visual do LoL
    render() {
        try {
            ...
            // Renderizar partículas de fundo no estilo LoL
            this.renderRuneterraBackground();
      ...
      // Renderizar a tela de carregamento estilo LoL
    renderLoading() {
        // O fundo já é renderizado pelo método renderRuneterraBackground()
        
        const loadingText = 'CARREGANDO RUNETERRA';
        const progress = (this.assets.loaded / this.assets.total) * 100;
        const dicas = [
            "Colete Códigos Perdidos para desbloquear novas habilidades",
            "Algumas regiões do mapa só podem ser acessadas após certo nível",
            "Experimente combinar diferentes elementos em seus feitiços",
            "A magia de código é mais forte durante a noite",
            "Interaja com NPCs para receber missões especiais",
            "Evolua suas habilidades para enfrentar inimigos mais poderosos",
            "Em Fehuna, programar é uma forma de magia ancestral"
        ];
        
        // Selecionar uma dica com base no tempo
        const dicaIndex = Math.floor(Date.now() / 5000) % dicas.length;
        const dicaAtual = dicas[dicaIndex];
        
        // Texto de carregamento com sombra
        ...
        this.ctx.fillStyle = this.LOL_COLORS.goldLight;
        this.ctx.textAlign = 'center';
        this.ctx.fillText(loadingText, this.canvas.width/2, this.canvas.height/2 - 80);
        ...
        // Dica estilo LoL
        this.ctx.font = 'italic 16px Arial, sans-serif';
        this.ctx.fillStyle = this.LOL_COLORS.runeterra;
        this.ctx.fillText('DICA DE INVOCADOR:', this.canvas.width/2, this.canvas.height/2 - 40);
        
        // Texto da dica com múltiplas linhas se necessário
        this.ctx.font = '16px Arial, sans-serif';
        this.ctx.fillStyle = this.LOL_COLORS.goldLight;
        
        // Quebrar a dica em linhas se for muito longa
        const maxWidth = this.canvas.width * 0.7;
        const palavras = dicaAtual.split(' ');
        let linhaAtual = '';
        let linhaY = this.canvas.height/2 - 15;
        
        palavras.forEach(palavra => {
            const testeLinha = linhaAtual + palavra + ' ';
            const larguraTexto = this.ctx.measureText(testeLinha).width;
            
            if (larguraTexto > maxWidth) {
                this.ctx.fillText(linhaAtual, this.canvas.width/2, linhaY);
                linhaAtual = palavra + ' ';
                linhaY += 20;
            } else {
        ...
        // Borda elegante
        this.ctx.strokeStyle = this.LOL_COLORS.gold;
        this.ctx.lineWidth = 1;
        this.ctx.strokeRect(barX - 1, barY - 1, barWidth + 2, barHeight + 2);
        
        // Progresso com gradiente
        const progressGradient = this.ctx.createLinearGradient(barX, barY, barX + barWidth * (progress/100), barY);
        progressGradient.addColorStop(0, this.LOL_COLORS.goldDark);
        progressGradient.addColorStop(0.5, this.LOL_COLORS.gold);
        progressGradient.addColorStop(1, this.LOL_COLORS.goldLight);
        
        this.ctx.fillStyle = progressGradient;
        this.ctx.fillRect(barX, barY, barWidth * (progress/100), barHeight);
        
        // Porcentagem com estilo
        this.ctx.font = 'bold 14px Arial, sans-serif';
        this.ctx.fillStyle = this.LOL_COLORS.goldLight;
        this.ctx.fillText(`${Math.floor(progress)}%`, this.canvas.width/2, barY + 30);
        
        // Adicionar ícones de artefatos carregados
        const totalIcons = 5;
        const iconSize = 24;
        const iconsWidth = totalIcons * (iconSize + 10);
        let iconX = (this.canvas.width - iconsWidth) / 2;
        const iconY = this.canvas.height/2 + 90;
        
        for (let i = 0; i < totalIcons; i++) {
            // Determinar se o ícone está carregado com base no progresso
            const isLoaded = (i + 1) * (100 / totalIcons) <= progress;
            ...
            if (isLoaded) {
                // Ícone carregado
                this.ctx.fillStyle = this.LOL_COLORS.gold;
                this.ctx.fill();
                
                // Símbolo interno
                this.ctx.fillStyle = this.LOL_COLORS.blue;
                this.ctx.font = '16px Arial, sans-serif';
                this.ctx.textAlign = 'center';
                this.ctx.textBaseline = 'middle';
                
                // Diferentes símbolos para cada tipo de recurso
                const symbols = ['⚔️', '🛡️', '✨', '📜', '🧙'];
                this.ctx.fillText(symbols[i], iconX + iconSize/2, iconY);
            } else {
                // Ícone não carregado
                this.ctx.fillStyle = 'rgba(200, 170, 110, 0.3)';
                this.ctx.fill();
            }
            
            // Borda do ícone
            this.ctx.strokeStyle = isLoaded ? this.LOL_COLORS.goldLight : 'rgba(200, 170, 110, 0.5)';
            this.ctx.lineWidth = 1;
            this.ctx.stroke();
        ...
        // Desenhar nome do jogo na parte inferior
        this.ctx.font = '14px Arial, sans-serif';
        this.ctx.fillStyle = this.LOL_COLORS.goldLight;
        this.ctx.textAlign = 'center';
        this.ctx.fillText('FEHUNA: CÓDIGOS PERDIDOS', this.canvas.width/2, this.canvas.height - 30);
        
        // Adicionar efeito de brilho pulsante na barra de progresso
        const pulseIntensity = (Math.sin(Date.now() * 0.01) + 1) * 0.5;
        this.ctx.globalAlpha = 0.2 + pulseIntensity * 0.3;
        this.ctx.fillStyle = this.LOL_COLORS.goldLight;
        this.ctx.fillRect(barX, barY + barHeight + 1, barWidth * (progress/100), 2);
        this.ctx.globalAlpha = 1.0;
    }
      // Renderizar o menu principal no estilo LoL
    renderMenu() {
        // O fundo já é renderizado pelo método renderRuneterraBackground()
        
        // Efeito de névoa superior
        const fogGradient = this.ctx.createLinearGradient(0, 0, 0, this.canvas.height/2);
        fogGradient.addColorStop(0, 'rgba(10, 20, 40, 0.7)');
        fogGradient.addColorStop(1, 'rgba(10, 20, 40, 0)');
        this.ctx.fillStyle = fogGradient;
        this.ctx.fillRect(0, 0, this.canvas.width, this.canvas.height/2);
        
        // Efeito de névoa inferior
        const fogGradient2 = this.ctx.createLinearGradient(0, this.canvas.height/2, 0, this.canvas.height);
        fogGradient2.addColorStop(0, 'rgba(10, 20, 40, 0)');
        fogGradient2.addColorStop(1, 'rgba(10, 20, 40, 0.7)');
        this.ctx.fillStyle = fogGradient2;
        this.ctx.fillRect(0, this.canvas.height/2, this.canvas.width, this.canvas.height/2);
        
        // Desenhar linhas decorativas estilo LoL
        this.ctx.strokeStyle = this.LOL_COLORS.gold;
        this.ctx.lineWidth = 2;
        
        // Linha horizontal superior
        this.ctx.beginPath();
        this.ctx.moveTo(this.canvas.width * 0.1, this.canvas.height * 0.2);
        this.ctx.lineTo(this.canvas.width * 0.9, this.canvas.height * 0.2);
        this.ctx.stroke();
        
        // Linha horizontal inferior
        this.ctx.beginPath();
        this.ctx.moveTo(this.canvas.width * 0.1, this.canvas.height * 0.8);
        this.ctx.lineTo(this.canvas.width * 0.9, this.canvas.height * 0.8);
        this.ctx.stroke();
        
        // Detalhes ornamentais nas laterais
        this.drawLolOrnament(this.canvas.width * 0.1, this.canvas.height * 0.2, true);
        this.drawLolOrnament(this.canvas.width * 0.9, this.canvas.height * 0.2, false);
        this.drawLolOrnament(this.canvas.width * 0.1, this.canvas.height * 0.8, true);
        this.drawLolOrnament(this.canvas.width * 0.9, this.canvas.height * 0.8, false);
        
        // Título com sombra e brilho estilo LoL
        // Sombra exterior
        this.ctx.shadowColor = 'rgba(0, 0, 0, 0.7)';
        ...
        // Efeito brilhante do título
        const pulseIntensity = (Math.sin(Date.now() * 0.002) + 1) * 0.5;
        const goldGradient = this.ctx.createLinearGradient(
            this.canvas.width/2 - 150, this.canvas.height/2 - 80, 
            this.canvas.width/2 + 150, this.canvas.height/2 - 30
        );
        goldGradient.addColorStop(0, this.LOL_COLORS.gold);
        goldGradient.addColorStop(0.5, this.LOL_COLORS.goldLight);
        goldGradient.addColorStop(1, this.LOL_COLORS.gold);
        
        this.ctx.fillStyle = goldGradient;
        this.ctx.textAlign = 'center';
        this.ctx.fillText('FEHUNA', this.canvas.width/2, this.canvas.height/2 - 80);
        
        // Remover sombra para o subtítulo
        this.ctx.shadowBlur = 0;
        this.ctx.shadowOffsetX = 0;
        this.ctx.shadowOffsetY = 0;
        ...
        // Desenhar contorno decorativo ao redor do título
        this.drawLolTitleFrame(
        ...
        // Menu de opções estilo LoL
        const menuOptions = [
            { text: 'INICIAR JORNADA', selected: true },
            { text: 'CARREGAR JOGO', selected: false },
            { text: 'OPÇÕES', selected: false }
        ];
        
        const menuY = this.canvas.height/2 + 60;
        const menuSpacing = 50;
        
        menuOptions.forEach((option, index) => {
            const y = menuY + (index * menuSpacing);
            
            // Desenhar botão no estilo LoL
            if (option.selected) {
                // Botão selecionado
                // Efeito de borda luminosa
                this.ctx.shadowColor = this.LOL_COLORS.gold;
                this.ctx.shadowBlur = 10;
                
                // Fundo do botão
                this.ctx.fillStyle = this.LOL_COLORS.goldDark;
                this.ctx.fillRect(this.canvas.width/2 - 140, y - 25, 280, 40);
                
                // Borda dourada
                this.ctx.strokeStyle = this.LOL_COLORS.gold;
                this.ctx.lineWidth = 2;
                this.ctx.strokeRect(this.canvas.width/2 - 140, y - 25, 280, 40);
                ...
                this.ctx.fillStyle = 'rgba(10, 20, 40, 0.6)';
                this.ctx.fillRect(this.canvas.width/2 - 120, y - 20, 240, 36);
                
                // Borda sutil
                this.ctx.strokeStyle = 'rgba(200, 170, 110, 0.3)';
                this.ctx.lineWidth = 1;
                this.ctx.strokeRect(this.canvas.width/2 - 120, y - 20, 240, 36);
            ...
            this.ctx.textAlign = 'center';
            this.ctx.fillText(option.text, this.canvas.width/2, y + 7);
        });
        
        // Instrução no estilo LoL
        const fadeEffect = (Math.sin(Date.now() * 0.005) + 1) * 0.5;
        this.ctx.globalAlpha = 0.3 + fadeEffect * 0.7;
        this.ctx.font = '16px Arial, sans-serif';
        this.ctx.fillStyle = this.LOL_COLORS.goldLight;
        this.ctx.fillText('Pressione ENTER para selecionar', this.canvas.width/2, this.canvas.height - 50);
        this.ctx.globalAlpha = 1.0;
        
        // Renderizar versão do jogo (estilo LoL)
        this.ctx.font = '12px Arial, sans-serif';
        this.ctx.fillStyle = this.LOL_COLORS.goldLight;
        this.ctx.textAlign = 'right';
        this.ctx.fillText('v1.0.0 Alpha', this.canvas.width - 20, this.canvas.height - 20);
        
        // Logo estilo LoL no canto
        this.drawFehunaMiniLogo(30, 30);
    }
    
    // Desenha um ornamento decorativo no estilo LoL
    drawLolOrnament(x, y, isLeft) {
        this.ctx.strokeStyle = this.LOL_COLORS.gold;
        this.ctx.lineWidth = 2;
        
        // Tamanho do ornamento
        const size = 30;
        ...
        this.ctx.beginPath();
        if (isLeft) {
            this.ctx.arc(x - size/2, y, size/4, 0, Math.PI * 2);
        } else {
            this.ctx.arc(x + size/2, y, size/4, 0, Math.PI * 2);
        }
        this.ctx.stroke();
    ...
    // Desenha uma moldura decorativa para o título no estilo LoL
    drawLolTitleFrame(x, y, width, height) {
        // Definir estilo
        this.ctx.strokeStyle = this.LOL_COLORS.goldDark;
        this.ctx.lineWidth = 3;
        
        // Desenhar retângulo base
        this.ctx.strokeRect(x, y, width, height);
        
        // Desenhar detalhes nos cantos
        const cornerSize = 15;
        ...
        this.ctx.lineWidth = 2;
        this.ctx.beginPath();
        this.ctx.moveTo(x + width/2 - 50, y);
        this.ctx.lineTo(x + width/2 + 50, y);
        this.ctx.stroke();
        
        this.ctx.beginPath();
        this.ctx.moveTo(x + width/2 - 50, y + height);
        this.ctx.lineTo(x + width/2 + 50, y + height);
        this.ctx.stroke();
    }
    
    // Desenha um mini-logo no estilo LoL
    drawFehunaMiniLogo(x, y) {
        // Fundo do logo
        this.ctx.beginPath();
        this.ctx.arc(x, y, 20, 0, Math.PI * 2);
        
        // Gradiente radial para o logo
        const gradient = this.ctx.createRadialGradient(x, y, 5, x, y, 20);
        gradient.addColorStop(0, this.LOL_COLORS.goldLight);
        gradient.addColorStop(1, this.LOL_COLORS.goldDark);
        ...
        // Borda do logo
        this.ctx.lineWidth = 2;
        this.ctx.strokeStyle = this.LOL_COLORS.goldLight;
        this.ctx.stroke();
 ...
 this.ctx.font = '14px Arial';
    ...
    renderUI() {
        ...
        // Renderizar barra de habilidades (skillbar) estilo LoL
        this.renderSkillbar();
    }
    ...
    // Renderiza o fundo inspirado em Runeterra (mundo de LoL)
    renderRuneterraBackground() {
        // Criar gradiente inspirado nas regiões de LoL com base no estado do jogo
        let gradient;
        
        switch(this.gameState) {
            case 'menu':
                // Gradiente inspirado em Demacia/Ionia
                gradient = this.ctx.createRadialGradient(
                    this.canvas.width/2, this.canvas.height/2, 10,
                    this.canvas.width/2, this.canvas.height/2, this.canvas.width
                );
                gradient.addColorStop(0, this.LOL_COLORS.goldLight);
                gradient.addColorStop(0.4, this.LOL_COLORS.gold);
                gradient.addColorStop(1, this.LOL_COLORS.blueDark);
                break;
                
            case 'battle':
                // Gradiente inspirado em Noxus
                gradient = this.ctx.createRadialGradient(
                    this.canvas.width/2, this.canvas.height/2, 10,
                    this.canvas.width/2, this.canvas.height/2, this.canvas.width
                );
                gradient.addColorStop(0, '#ff6666');
                gradient.addColorStop(0.7, this.LOL_COLORS.noxus);
                gradient.addColorStop(1, this.LOL_COLORS.blueDark);
                break;
                
            case 'playing':
                // Gradiente baseado na hora do dia no jogo
                const hour = this.world ? this.world.timeOfDay : 12;
                
                if (hour >= 5 && hour < 10) {
                    // Nascer do sol - Ionia
                    gradient = this.ctx.createLinearGradient(0, 0, 0, this.canvas.height);
                    gradient.addColorStop(0, '#fa9393');
                    gradient.addColorStop(0.3, this.LOL_COLORS.ionia);
                    gradient.addColorStop(1, this.LOL_COLORS.runeterra);
                } else if (hour >= 10 && hour < 17) {
                    // Dia - Demacia
                    gradient = this.ctx.createLinearGradient(0, 0, 0, this.canvas.height);
                    gradient.addColorStop(0, '#87ceeb');
                    gradient.addColorStop(0.6, this.LOL_COLORS.blueLight);
                    gradient.addColorStop(1, this.LOL_COLORS.demacia);
                    ...
                    // Noite - Shadow Isles
                    gradient = this.ctx.createLinearGradient(0, 0, 0, this.canvas.height);
                    gradient.addColorStop(0, this.LOL_COLORS.blueDark);
                    gradient.addColorStop(0.7, this.LOL_COLORS.shadow);
            ...
            case 'loading':
            default:
                // Gradiente padrão - Piltover/Zaun
                gradient = this.ctx.createLinearGradient(0, 0, 0, this.canvas.height);
                gradient.addColorStop(0, this.LOL_COLORS.blueDark);
                gradient.addColorStop(0.5, this.LOL_COLORS.blue);
                gradient.addColorStop(1, this.LOL_COLORS.blue);
                break;
        }
        ...
        // Adicionar padrão de runas suaves no fundo
        this.renderRunePatterns();
    }
    
    // Renderiza padrões de runas no estilo de LoL
    renderRunePatterns() {
        this.ctx.strokeStyle = runeColor;
        
        // Definir quantidade de runas com base no tamanho do canvas
        const runeCount = Math.floor((this.canvas.width * this.canvas.height) / 25000);
        ...
        for (let i = 0; i < runeCount; i++) {
            ...
            // Escolher forma de runa aleatória
            const runeType = Math.floor(Math.random() * 5);
            
            this.ctx.beginPath();
            switch (runeType) {
                case 0: // Círculo - Targon
                    this.ctx.arc(x, y, size/2, 0, Math.PI * 2);
                    break;
                case 1: // Triângulo - Freljord
                    this.ctx.moveTo(x, y - size/2);
                    this.ctx.lineTo(x + size/2, y + size/2);
                    this.ctx.lineTo(x - size/2, y + size/2);
                    this.ctx.closePath();
                    break;
                case 2: // Hexágono - Piltover
                    for (let j = 0; j < 6; j++) {
                    ...
                    this.ctx.closePath();
                    break;
                case 3: // Quadrado com corte - Noxus
                    this.ctx.rect(x - size/3, y - size/3, size*2/3, size*2/3);
                    break;
                case 4: // Crescente - Ionia
                    this.ctx.arc(x, y, size/2, 0, Math.PI);
                    this.ctx.arc(x, y - size/4, size/3, Math.PI, Math.PI * 2);
                    break;
            }
    ...
    // Renderiza efeitos de partículas no estilo de LoL
    renderParticleEffects() {
        // Atualizar e renderizar partículas existentes
        for (let i = this.effects.particles.length - 1; i >= 0; i--) {
            const particle = this.effects.particles[i];
            
            // Atualizar tempo de vida
            particle.life -= 0.016; // Aproximadamente 60 FPS
            ...
            // Aplicar gravidade ou outra física se necessário
            if (particle.gravity) {
                particle.vy += 0.1;
            }
            
            // Atualizar tamanho baseado no tempo de vida
            const sizeRatio = particle.life / particle.initialLife;
            const currentSize = particle.size * sizeRatio;
            
            // Atualizar opacidade baseado no tempo de vida
            const opacity = particle.fadeOut ? sizeRatio : 1;
            
            // Renderizar a partícula
            ...
            switch (particle.type) {
                ...
                case 'spark':
                    this.ctx.beginPath();
                    this.ctx.moveTo(particle.x, particle.y - currentSize);
                    this.ctx.lineTo(particle.x + currentSize, particle.y);
                    this.ctx.lineTo(particle.x, particle.y + currentSize);
                    this.ctx.lineTo(particle.x - currentSize, particle.y);
                    this.ctx.closePath();
                    this.ctx.fillStyle = particle.color;
                    this.ctx.fill();
                ...
                case 'rune':
                    this.ctx.font = `${Math.floor(currentSize * 2)}px Arial`;
                    this.ctx.fillStyle = particle.color;
        ...
        // Se estivermos no estado de menu, adicionar algumas partículas decorativas
        if (this.gameState === 'menu' && Math.random() < 0.05) {
            this.addParticle({
                ...
                color: this.LOL_COLORS.gold,
                life: 2 + Math.random() * 3,
                type: Math.random() > 0.7 ? 'rune' : 'circle',
                fadeOut: true
            });
        }
        
        // Se estivermos em batalha, adicionar efeitos de energia
        if (this.gameState === 'battle' && Math.random() < 0.2) {
            const side = Math.random() > 0.5 ? 'player' : 'enemy';
            const baseX = side === 'player' ? this.canvas.width * 0.25 : this.canvas.width * 0.75;
            ...
            this.addParticle({
                ...
                color: side === 'player' ? this.LOL_COLORS.blueLight : this.LOL_COLORS.noxus,
    ...
    // Adiciona novas partículas ao sistema
    addParticle(config) {
        const defaultConfig = {
            x: 0,
            y: 0,
    ...
    // Adiciona efeito de trilha atrás de um objeto em movimento
    addTrailEffect(x, y, color, size = 5) {
        this.addParticle({
            x,
            y,
            vx: 0,
