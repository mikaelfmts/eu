...
window.addEventListener('load', () => {
    ...
    // Carregar imagem de perfil do cache imediatamente para feedback visual rápido
    ...
    setTimeout(() => {
        ...
        // Buscar dados atualizados do GitHub - irá também atualizar a foto de perfil
        fetchGitHubData();
...
function createSkillCards(languageStats, technologyStats, totalBytes, fromCache = false) {
    ...
    Object.entries(allSkills).forEach(([name, data]) => {
        skillsHTML += `
            <div class="skill-card" 
                onmouseover="this.style.transform='translateY(-8px)'; this.style.boxShadow='0 15px 30px rgba(0, 0, 0, 0.5), 0 8px 15px rgba(200, 170, 110, 0.3)';" 
                onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='0 10px 20px rgba(0, 0, 0, 0.3), 0 4px 10px rgba(200, 170, 110, 0.15)';"
                style="
                    ...
                    position: relative;
                    overflow: hidden;
                    box-shadow: 0 10px 20px rgba(0, 0, 0, 0.3), 0 4px 10px rgba(200, 170, 110, 0.15);
                ...
                <div style="
                    ...
                    width: 100%;
                    height: 5px;
                    background: linear-gradient(90deg, ${getSkillColor(name)} 0%, #d4af37 100%);
                    opacity: 0.8;
                "></div>
                <div class="skill-header" style="
                    ...
                    gap: 1rem;
                ">
                    <span class="skill-icon" style="
                        background: linear-gradient(135deg, ${getSkillColor(name)} 0%, rgba(200, 170, 110, 0.8) 100%);
                        width: 48px;
                        height: 48px;
                        border-radius: 12px;
                        display: flex;
                    ...
                    "><i class="${getSkillIcon(name)}" style="font-size: 1.5rem; color: #111;"></i></span>
                    <h3 style="
                        margin: 0;
                        font-size: 1.35rem;
                ...
                <div class="skill-footer" style="
                    display: flex;
                    justify-content: space-between;
                    align-items: center;
                    margin-top: 0.8rem;
                ">
                    <div class="skill-percentage" style="
                        font-size: 1.2rem;
                        font-weight: 700;
                        color: var(--primary-color);
                        text-shadow: 0 1px 2px rgba(0, 0, 0, 0.3);
                    ">${data.percentage}%</div>
                    <div class="skill-details" style="
                        font-size: 0.9rem;
                        ...
                        gap: 1rem;
                    ">
                        ${data.bytes ? `<span><i class="fas fa-code" style="margin-right: 5px;"></i>${formatBytes(data.bytes)}</span>` : ''}
                        <span><i class="fas fa-tag" style="margin-right: 5px;"></i>${data.type}</span>
                    </div>
                </div>
            </div>
        `;
    ...
    // Adicionar animação de shimmer/brilho se ainda não existir
    if (!document.getElementById('skills-animation-style')) {
        const animationStyle = document.createElement('style');
        animationStyle.id = 'skills-animation-style';
...
async function fetchGitHubProfile(username) {
    ...
    // Primeiro tentamos carregar do localStorage para feedback rápido
    loadProfilePictureFromStorage();
    ...
    try {
        ...
        // Atualizar perfil GitHub na seção específica
        updateGitHubProfile(profileData);
        
        // Atualizar a foto de perfil em todas as páginas
        if (profileData.avatar_url) {
            updateProfilePicture(profileData.avatar_url);
        }
        ...
        // Atualizar a foto de perfil em todas as páginas mesmo no fallback
        if (fallbackProfile.avatar_url) {
            updateProfilePicture(fallbackProfile.avatar_url);
        }
...
// Função para atualizar a foto de perfil com avatar do GitHub
function updateProfilePicture(avatarUrl) {
    if (!avatarUrl) return;
    
    console.log('🖼️ Atualizando foto de perfil com avatar do GitHub:', avatarUrl);
    
    // Buscar todos os elementos da foto de perfil em todas as páginas
    const profilePictureElements = document.querySelectorAll('.foto-perfil img');
    
    if (profilePictureElements.length === 0) {
        console.warn('⚠️ Nenhum elemento de foto de perfil encontrado no DOM');
        return;
    }
    
    // Melhorar estilo do container da foto de perfil
    const profileContainers = document.querySelectorAll('.foto-perfil');
    profileContainers.forEach(container => {
        container.style.borderRadius = '50%';
        container.style.overflow = 'hidden';
        container.style.boxShadow = '0 8px 25px rgba(0, 0, 0, 0.4), 0 4px 12px rgba(200, 170, 110, 0.3)';
        container.style.border = '3px solid rgba(200, 170, 110, 0.7)';
        container.style.transition = 'all 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275)';
        
        // Adicionar efeito de hover
        container.addEventListener('mouseenter', () => {
            container.style.transform = 'scale(1.08)';
            container.style.boxShadow = '0 12px 30px rgba(0, 0, 0, 0.5), 0 8px 20px rgba(200, 170, 110, 0.4)';
            container.style.border = '3px solid rgba(200, 170, 110, 1)';
        });
    ...
    // Atualizar cada elemento de foto de perfil
    profilePictureElements.forEach(imgElement => {
        // Adicionar efeito de fade-out antes de trocar a imagem
        imgElement.style.transition = 'all 0.4s ease-out';
        imgElement.style.opacity = '0.3';
        imgElement.style.transform = 'scale(0.9)';
        
        // Após um breve delay, trocar a imagem e fazer fade-in
        setTimeout(() => {
            imgElement.src = avatarUrl;
            imgElement.alt = 'Foto de Perfil GitHub';
            imgElement.onload = function() {
                ...
                // Adicionar borda dourada pulsante para sinalizar que a imagem foi atualizada
                const parentElement = imgElement.parentElement;
                if (parentElement) {
                    parentElement.style.boxShadow = '0 0 20px rgba(200, 170, 110, 0.8)';
                    parentElement.style.animation = 'pulse-border 2s ease-in-out';
                    
                    // Adicionar um pequeno emblema do GitHub
                    const githubBadge = document.createElement('div');
                    githubBadge.style.position = 'absolute';
                    githubBadge.style.bottom = '0';
                    githubBadge.style.right = '0';
                    githubBadge.style.background = 'rgba(0, 0, 0, 0.7)';
                    githubBadge.style.color = '#c8aa6e';
                    githubBadge.style.borderRadius = '50%';
                    githubBadge.style.width = '24px';
                    githubBadge.style.height = '24px';
                    githubBadge.style.display = 'flex';
                    githubBadge.style.alignItems = 'center';
                    githubBadge.style.justifyContent = 'center';
                    githubBadge.style.fontSize = '14px';
                    githubBadge.style.boxShadow = '0 2px 5px rgba(0,0,0,0.3)';
                    githubBadge.innerHTML = '<i class="fab fa-github"></i>';
                    
                    // Verificar se o badge já existe
                    if (!parentElement.querySelector('.github-badge')) {
                        githubBadge.classList.add('github-badge');
                        parentElement.style.position = 'relative';
                        parentElement.appendChild(githubBadge);
                    }
                    
                    // Remover efeito após a animação
                    setTimeout(() => {
    ...
    // Salvar URL da imagem no localStorage para persistência
    try {
        localStorage.setItem('github-profile-picture', avatarUrl);
        console.log('✅ URL do avatar salva no localStorage para uso offline');
    } catch (error) {
        console.warn('⚠️ Não foi possível salvar avatar no localStorage:', error);
    }
    
    // Adicionar animação de pulsar borda se ainda não existir
    if (!document.getElementById('profile-picture-animation')) {
        const style = document.createElement('style');
        style.id = 'profile-picture-animation';
        style.textContent = `
            @keyframes pulse-border {
                0% { box-shadow: 0 0 0 0 rgba(200, 170, 110, 0.8); }
                70% { box-shadow: 0 0 0 20px rgba(200, 170, 110, 0); }
...
// Função para carregar avatar do GitHub offline (do localStorage)
function loadProfilePictureFromStorage() {
    try {
        const savedAvatarUrl = localStorage.getItem('github-profile-picture');
        if (savedAvatarUrl) {
            console.log('🔄 Carregando avatar do localStorage (modo offline)');
            updateProfilePicture(savedAvatarUrl);
            return true;
        }
    } catch (error) {
        console.warn('⚠️ Erro ao carregar avatar do localStorage:', error);
    }
    return false;
}
