function toggleMenu() {
    ...
    console.log("Menu está " + (isOpen ? "aberto" : "fechado"));
...
// Inicializar controles superiores
document.addEventListener("DOMContentLoaded", function() {
    // Inicializar botões de controle do topo
    initTopControls();
...
if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => {
        navigator.serviceWorker.register('/sw.js')
            ...
            .catch(error => {
                console.warn('ServiceWorker não pôde ser registrado:', error.message);
            });
    });
}

// Função para inicializar os botões de controle agora no menu lateral
function initTopControls() {
    ...
    if (colorBtn && colorMenu) {
        // Toggle do menu de cores ao clicar no botão
        colorBtn.addEventListener('click', function(e) {
            e.stopPropagation();
            const isVisible = colorMenu.style.display === 'flex';
            colorMenu.style.display = isVisible ? 'none' : 'flex';
        });
    ...
    // Botão de idioma - apenas simula a funcionalidade
    const langBtn = document.getElementById('language-btn');
    if (langBtn) {
        langBtn.addEventListener('click', function() {
            // Aqui você pode implementar um menu de idiomas ou alternar entre idiomas definidos
    ...
    if (themeToggle) {
        ...
        // Configurar ícone correto no carregamento
        const isLightMode = document.body.classList.contains('light-mode');
        const themeIcon = document.getElementById('theme-icon');
        if (themeIcon) {
            themeIcon.className = isLightMode ? 'fas fa-moon' : 'fas fa-sun';
    ...
    // Manter o menu de cores visível ao clicar dentro dele
    if (colorMenu) {
