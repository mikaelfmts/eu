...
async function fetchGitHubData() {
    ...
    try {
        ...
        // Buscar TODOS os repositórios para análise completa de skills
        ...
        // Verificar se allRepos é um array
        if (!Array.isArray(allRepos)) {
            throw new Error('Invalid response from GitHub API');
        }
        
        // Buscar apenas os 5 mais recentes para exibição
        ...
        // Tentar exibir repositórios mesmo com erro nas skills
        try {
            const fallbackRepos = await fetch(`https://api.github.com/users/mikaelfmts/repos?per_page=5`);
            if (fallbackRepos.ok) {
                const repos = await fallbackRepos.json();
                if (Array.isArray(repos)) {
                    console.log('Exibindo repositórios de fallback...');
                    updateGitHubRepos(repos);
                }
            }
        } catch (fallbackError) {
            console.warn('Erro no fallback dos repositórios:', fallbackError);
        }
    ...
    try {
        ...
        messagesListener = onSnapshot(q, (snapshot) => {
            ...
            if (snapshot.empty) {
                ...
                // Converter para array e reverter a ordem (pois vem desc do Firebase)
                const messages = [];
                snapshot.forEach((doc) => {
                    messages.push({ data: doc.data(), id: doc.id });
                });
                
                // Exibir mensagens na ordem correta (mais antigas primeiro)
