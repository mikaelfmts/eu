...
// Função para verificar acesso à página de geração de currículo
export function checkCurriculoGeneratorAccess() {
    return new Promise((resolve, reject) => {
        onAuthStateChanged(auth, (user) => {
            if (user) {
                // Verificar se é o email autorizado
                if (user.email === 'mikaelfmts@gmail.com') {
                    console.log("Acesso autorizado ao gerador de currículo");
                    resolve(user);
                } else {
                    // Email não autorizado, redireciona para admin
                    console.log("Email não autorizado para o gerador de currículo");
                    alert("Você não tem permissão para acessar esta página.");
                    window.location.href = 'admin.html';
                    reject('Acesso não autorizado');
                }
            } else {
                // Não está logado, redireciona para login
                console.log("Usuário não autenticado, redirecionando...");
