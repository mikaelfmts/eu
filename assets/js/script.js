...
function initSnakeGame() {
    ...
    let isPaused = false;
    let obstacles = [];
    let particles = []; // Array para partículas de efeito
    let trailPositions = []; // Array para rastro da cobra
    let shakeIntensity = 0; // Intensidade do screen shake
    ...
    function gameLoop() {
        ...
        if (eatFood()) {
            // Criar partículas de efeito quando come
    ...
    function createFoodParticles(x, y) {
        for (let i = 0; i < 8; i++) {
            particles.push({
                ...
                life: 1.0,
                color: `hsl(${Math.random() * 60 + 0}, 100%, 60%)` // Tons de vermelho/laranja
            });
        }
    }
    ...
    function updateTrail() {
        for (let i = trailPositions.length - 1; i >= 0; i--) {
            trailPositions[i].life -= 0.05;
            if (trailPositions[i].life <= 0) {
                trailPositions.splice(i, 1);
            }
        }
        
        // Limitar número de pontos do rastro
        if (trailPositions.length > 15) {
            trailPositions.splice(0, trailPositions.length - 15);
        }
    }
    ...
    function drawTrail() {
        ...
        for (let i = 1; i < trailPositions.length; i++) {
            ...
            ctx.globalAlpha = current.life * 0.5;
            ctx.beginPath();
    ...
    function draw() {
        ...
        // Desenhar cobra com gradiente e bordas arredondadas
        for (let i = 0; i < snake.length; i++) {
            ...
            if (i === 0) {
                // Cabeça da cobra com gradiente radial e brilho
                ...
                ctx.shadowBlur = 8;
                ctx.beginPath();
                ctx.roundRect(x + 1.5, y + 1.5, size, size, 6);
                ctx.fill();
                ...
                // Corpo da cobra com gradiente e transparência gradual
                const bodyAlpha = Math.max(0.4, 1 - (i / snake.length) * 0.6);
                const bodyGradient = ctx.createLinearGradient(x, y, x + size, y + size);
                bodyGradient.addColorStop(0, `rgba(139, 195, 74, ${bodyAlpha})`);
                bodyGradient.addColorStop(1, `rgba(104, 159, 56, ${bodyAlpha})`);
                
                ctx.fillStyle = bodyGradient;
                ctx.shadowColor = '#8BC34A';
                ctx.shadowBlur = 4;
                ctx.beginPath();
                ctx.roundRect(x + 1.5, y + 1.5, size, size, 4);
                ctx.fill();
            }
        }
        
        // Desenhar comida com animação pulsante
