...
async function loadCertificates() {
    try {
        ...
        const q = query(certificatesRef, orderBy('dataConclusao', 'desc'));
...
function updateMaintenanceUI() {
    ...
    if (!statusText || !toggleButton) return;
    ...
    if (maintenanceData.ativo) {
        ...
        if (maintenanceForm) {
            ...
            document.getElementById('maintenance-title').value = maintenanceData.titulo || '';
            document.getElementById('maintenance-message').value = maintenanceData.mensagem || '';
            document.getElementById('maintenance-eta').value = maintenanceData.previsao || '';
        }
    } else {
...
async function toggleMaintenance() {
    try {
        ...
        if (novoStatus) {
            dadosAtualizacao.titulo = document.getElementById('maintenance-title').value.trim() || 'Site em Manutenção';
            dadosAtualizacao.mensagem = document.getElementById('maintenance-message').value.trim() || 'Estamos realizando melhorias no site. Voltamos em breve!';
            dadosAtualizacao.previsao = document.getElementById('maintenance-eta').value.trim() || '';
        }
          const maintenanceRef = doc(db, 'configuracoes', 'manutencao');
        ...
        showError('Erro ao alterar status de manutenção');
    }
}
