...
async function syncProfilePhoto() {
    try {
        ...
        // Esconder as imagens de perfil durante o carregamento
        ...
        if (response.ok) {
            ...
            if (data.avatar_url) {
                profilePhotos.forEach(img => {
                    if (img) {
                        // Atualiza a src somente após o carregamento da nova imagem
                        const newImg = new Image();
                        newImg.onload = () => {
                            img.src = data.avatar_url;
                            img.style.opacity = '1';
                            
                            // Remover indicadores de carregamento quando a imagem estiver pronta
                            const loadingIndicators = document.querySelectorAll('.profile-loading');
        ...
        // Em caso de erro, restaurar a opacidade
        const profilePhotos = document.querySelectorAll('.foto-perfil img, #profile-photo');
        profilePhotos.forEach(img => {
            if (img) img.style.opacity = '1';
        });
        console.log('⚠️ Erro ao sincronizar foto do perfil:', error);
    }
}

// Garantir que a sincronização da foto funcione em todas as páginas
document.addEventListener('DOMContentLoaded', () => {
    // Adicionar um efeito de carregamento à foto do perfil
    const profilePhotos = document.querySelectorAll('.foto-perfil');
    profilePhotos.forEach(container => {
        if (container) {                // Adicionar indicador de carregamento elegante
...
window.addEventListener('load', () => {
    ...
    if (!document.getElementById('profile-animation-style')) {
        const styleTag = document.createElement('style');
        styleTag.id = 'profile-animation-style';
        styleTag.textContent = `
            ...
            @keyframes pulse {
                ...
                70% { box-shadow: 0 0 0 10px rgba(200, 170, 110, 0); }
