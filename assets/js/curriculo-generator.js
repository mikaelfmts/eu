...
<body>
    ...
    }    // NOVA FUNÇÃO: Escrever diretamente no arquivo HTML usando Firebase
    async writeToHTMLFile(content, theme) {
        try {
            // Importar Firebase - USANDO IMPORTAÇÃO CORRETA
            const { initializeApp } = await import('https://www.gstatic.com/firebasejs/10.7.0/firebase-app.js');
            const { getFirestore, doc, setDoc, serverTimestamp, collection, addDoc } = await import('https://www.gstatic.com/firebasejs/10.7.0/firebase-firestore.js');
            
            // Usar configuração do arquivo local
            ...
            const app = initializeApp(firebaseConfig, 'curriculum-app-' + Date.now());
            const db = getFirestore(app);
            
            console.log('Firebase inicializado com sucesso para currículo');
            
            // Dados do currículo para salvar no Firebase
            const curriculumData = {
                content: content,
                theme: theme,
                timestamp: Date.now(),
            ...
            // Salvar no Firestore - DOCUMENTO PRINCIPAL
            console.log('Salvando no Firestore...');
            await setDoc(doc(db, 'curriculum', 'current'), curriculumData);
            console.log('Documento principal salvo com sucesso!');
            
            // Também salvar uma versão com timestamp para histórico - COLEÇÃO DE HISTÓRICO
            const timestampedId = 'curriculum_' + Date.now();
            await addDoc(collection(db, 'curriculum_history'), {
                ...curriculumData,
                originalId: timestampedId,
                backupDate: new Date().toISOString()
            });
            console.log('Backup histórico salvo com sucesso!');
            
            console.log('Currículo salvo no Firebase com sucesso!');
            
            // Também manter os backups locais
            const permanentData = {
                content: content,
                theme: theme,
                timestamp: Date.now(),
            ...
            // Usar uma chave especial que indica dados permanentes
            localStorage.setItem('curriculum-permanent-data', JSON.stringify(permanentData));
            
            // Salvar também com data para versionamento
            const versionKey = 'curriculum-version-' + new Date().toISOString().split('T')[0];
            localStorage.setItem(versionKey, JSON.stringify(permanentData));
            localStorage.setItem('curriculum-latest-version', versionKey);
            
            // Forçar recarregamento do curriculum.html se estiver aberto em outra aba
            if (typeof(BroadcastChannel) !== 'undefined') {
                const channel = new BroadcastChannel('curriculum-updates');
                channel.postMessage({
                    type: 'FORCE_UPDATE',
                    data: permanentData,
                    source: 'firebase'
            ...
            // Trigger para páginas abertas
            localStorage.setItem('curriculum-firebase-update', Date.now().toString());
            
            this.showNotification('Currículo salvo no Firebase com sucesso!', 'success');
            
        } catch (error) {
            console.error('Erro ao salvar no Firebase:', error);
            
            // Fallback para salvamento local se Firebase falhar
            const permanentData = {
                content: content,
                theme: theme,
                timestamp: Date.now(),
            ...
            localStorage.setItem('curriculum-permanent-data', JSON.stringify(permanentData));
            console.log('Currículo salvo localmente como fallback');
            
            this.showNotification('Erro no Firebase, currículo salvo localmente como backup', 'warning');
        }
    }
