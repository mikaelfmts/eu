...
async function uploadMediaFile(file) {
    try {
        ...
        // Usar versão avançada com fallback CORS
        return await uploadWithCORSFallback(file);
        ...
        if (error.message.includes('CORS') || error.message.includes('blocked') || 
            error.message.includes('preflight') || error.code === 'storage/unknown') {
            throw new Error(`🚨 ERRO DE CORS DETECTADO!\n\n` +
                `CAUSA: As regras do Firebase Storage não foram aplicadas no console.\n\n` +
                `SOLUÇÃO RÁPIDA:\n` +
                `1. Abra: https://console.firebase.google.com\n` +
                `2. Projeto: mikaelfmts\n` +
                `3. Storage → Rules\n` +
                `4. Cole o conteúdo do arquivo firebase-rules.txt\n` +
                `5. Clique em "Publicar"\n` +
                `6. Aguarde 5-10 minutos\n\n` +
                `Use debug-cors.html para testar a correção.`);
        }
...
// Função auxiliar avançada com fallback para CORS
