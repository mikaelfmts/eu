...
function initializeGenerator() {
    console.log('🚀 Inicializando sistema do gerador de currículo...');
    
    try {
        generator = new CurriculumGenerator();
        console.log('✅ Generator criado com sucesso');
        
        // Verificar se o generator foi criado corretamente
        if (generator) {
            console.log('✅ Generator está disponível globalmente');
            
            // Expor o generator globalmente para debug
            window.generator = generator;
            
            // Executar diagnóstico após 2 segundos
            setTimeout(() => {
                if (typeof generator.runDiagnostic === 'function') {
                    generator.runDiagnostic();
                }
            }, 2000);
        } else {
            console.error('❌ Generator não foi criado corretamente');
        }
    } catch (error) {
        console.error('❌ Erro ao inicializar generator:', error);
    }
}
...
async function saveCurriculum() {
    ...
    if (!generator) {
        console.error('❌ Generator não inicializado');
    ...
    try {
        ...
        await generator.saveCurriculumData(data);
        generator.showNotification('✅ Currículo salvo com sucesso!', 'success');
        console.log('✅ Salvamento concluído');
    } catch (error) {
        console.error('❌ Erro ao salvar currículo:', error);
        generator.showNotification('❌ Erro ao salvar: ' + error.message, 'error');
    }
}
...
async function loadCurriculum() {
    ...
    try {
        await generator.loadSavedCurriculum();
        generator.showNotification('✅ Currículo carregado com sucesso!', 'success');
        console.log('✅ Carregamento concluído');
    } catch (error) {
        console.error('❌ Erro ao carregar currículo:', error);
        generator.showNotification('❌ Erro ao carregar: ' + error.message, 'error');
    }
}

function getFirebaseStatus() {
    if (!generator) {
        console.warn('⚠️ Generator não inicializado');
        return 'Sistema não inicializado';
    }
    
    if (!generator.firebaseService) {
        console.warn('⚠️ Firebase service não disponível');
        return 'Firebase não disponível';
    }
    
    const status = generator.firebaseService.isConnected ? 'Conectado ✅' : 'Desconectado ❌';
