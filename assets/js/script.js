...
function loadChatMessages() {
    ...
    try {
        ...
        messagesListener = onSnapshot(q, (snapshot) => {
            ...
            // Processar todas as mensagens atuais primeiro se o snapshot inicial
            if (snapshot.docs && snapshot.docs.length > 0 && !document.querySelector('[data-message-id]')) {
                ...
                // Mostrar todas as mensagens atuais
                snapshot.docs.forEach(doc => {
                    displayMessage(doc.data(), doc.id);
                });
            }
            
            // Processar alterações específicas para otimizar render
            snapshot.docChanges().forEach((change) => {
                ...
                if (change.type === "added" || change.type === "modified") {
                    ...
                    // Se for uma mensagem modificada com resposta, notifica o usuário
                    if (change.type === "modified" && data.resposta && existingMsg && !existingMsg.querySelector('.admin-response')) {
...
function displayMessage(messageData, messageId) {
    ...
    // Se a mensagem já existe e tem o mesmo status de resposta, retornar
    // Mas se tiver uma resposta nova, vamos recriá-la
    if (existingMsg) {
        const hasResponseElement = existingMsg.querySelector('.admin-response');
        const hasResponse = messageData.resposta && messageData.resposta.trim() !== '';
        
        if ((hasResponseElement && hasResponse) || (!hasResponseElement && !hasResponse)) {
            return; // Não precisa atualizar
        }
        
        // Se chegou aqui, a mensagem mudou (recebeu ou perdeu resposta)
        existingMsg.remove();
    }
    ...
    // Notificar o usuário se for uma nova resposta
    if (messageData.resposta && existingMsg && !existingMsg.querySelector('.admin-response')) {
        addSystemMessage('Nova resposta recebida de Mikael!', 'success');
    }
}
...
function toggleChat() {
    ...
    if (!isVisible && currentUserName) {
        // Parar o listener atual se existir
        if (messagesListener) {
            messagesListener();
