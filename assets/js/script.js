...
function loadChatMessages() {
    ...
    try {
        ...
        messagesListener = onSnapshot(q, (snapshot) => {
            ...
            // Primeira carga: carregar todas as mensagens existentes
            if (!chatMessages.dataset.initialized) {
                ...
                // Cargas subsequentes: processar apenas mudanças
                snapshot.docChanges().forEach((change) => {
                    const data = change.doc.data();
                    const messageId = change.doc.id;
                    
                    if (change.type === 'added') {
                        // Nova mensagem - apenas adicionar se não existir
                        if (!document.querySelector(`[data-message-id="${messageId}"]`)) {
                            displayMessage(data, messageId);
                        }
                    } else if (change.type === 'modified') {
                        // Mensagem modificada - atualizar a mensagem existente
