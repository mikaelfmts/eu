...
<body>
    ...
    }    // FUNÇÃO DEFINITIVA: Salvar no Firebase Firestore com Autenticação Anônima
    async writeToHTMLFile(content, theme) {
        console.log('🔥 SALVANDO NO FIREBASE FIRESTORE COM AUTH...');
          try {
            // Importar Firebase sem Auth (modo público)
            const { initializeApp } = await import('https://www.gstatic.com/firebasejs/10.7.0/firebase-app.js');
            const { getFirestore, doc, setDoc, serverTimestamp, connectFirestoreEmulator } = await import('https://www.gstatic.com/firebasejs/10.7.0/firebase-firestore.js');
            ...
            console.log('✅ Firebase inicializado em modo público...');
            ...
            const curriculumData = {
              ...
              // Salvar no Firestore com acesso público
            await setDoc(doc(db, 'curriculum', 'current'), curriculumData, { merge: true });
            
            console.log('✅ Currículo salvo no Firebase com sucesso!');
            this.showNotification('✅ Currículo salvo no Firebase! Será exibido automaticamente em curriculum.html', 'success');
            
            // Salvar também no localStorage como backup
            localStorage.setItem('curriculum-firebase-backup', JSON.stringify(curriculumData));
            localStorage.setItem('curriculum-last-save', Date.now().toString());
            
        } catch (error) {
            console.error('❌ Erro ao salvar no Firebase:', error);
            
            // FALLBACK ROBUSTO: Salvar no localStorage com estrutura especial
            const backupData = {
                content: content,
                theme: theme,
                timestamp: Date.now(),
            ...
            // Salvar com múltiplas chaves para garantir persistência
            localStorage.setItem('curriculum-local-primary', JSON.stringify(backupData));
            localStorage.setItem('curriculum-permanent-data', JSON.stringify(backupData));
            localStorage.setItem('curriculum-last-save', Date.now().toString());
            
            console.log('💾 Currículo salvo no localStorage como fallback');
            this.showNotification('⚠️ Firebase indisponível. Currículo salvo localmente!', 'warning');
        }
    }
