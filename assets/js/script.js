    ...
    try {
        ...
        if (!response.ok) {
            if (response.status === 403) {
                console.warn('Rate limit atingido, usando dados padrão');
...
async function fetchGitHubRepositories(username) {
    ...
    try {
        ...
        if (!response.ok) {
            if (response.status === 403) {
                console.warn('Rate limit atingido para repositórios');
                throw new Error('Rate limit exceeded');
            }
            throw new Error(`Erro na API do GitHub: ${response.status} ${response.statusText}`);
        }
...
async function loadLinkedInProfile() {
    ...
    try {
        // Usar dados de fallback (API do LinkedIn requer OAuth complexo)
        ...
        const profileHTML = `
            <div class="linkedin-card">
                <div class="linkedin-header">
                    <img src="${profileData.avatar}" alt="${profileData.name}" class="linkedin-avatar">
                    <div class="linkedin-info">
                        <h3>${profileData.name}</h3>
                        <div class="linkedin-title">${profileData.title}</div>
                ...
                <div class="linkedin-stats">
                    <div class="linkedin-stat">
                        <span class="linkedin-stat-value">${profileData.connections}</span>
                        <span class="linkedin-stat-label">Conexões</span>
                    </div>
                    <div class="linkedin-stat">
                        <span class="linkedin-stat-value">${profileData.skills.length}</span>
                        <span class="linkedin-stat-label">Habilidades</span>
                    </div>
                    <div class="linkedin-stat">
                        <span class="linkedin-stat-value">${profileData.projects.length}</span>
...
// Adicionar LinkedIn ao carregamento da página
document.addEventListener('DOMContentLoaded', () => {
    // ...existing code...
    
    // Verificar se estamos na página principal e carregar LinkedIn
    setTimeout(() => {
