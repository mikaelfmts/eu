...
function loadChatMessages() {
    ...
    // Limpar mensagens anteriores, exceto as do sistema
    const chatMessages = document.getElementById('chat-messages');
    if (!chatMessages) return;
    
    // Verificar se já existe um indicador de carregamento
    let loadingMessage = document.getElementById('loading-messages');
    if (!loadingMessage) {
        // Se não existir, criar um novo
        loadingMessage = document.createElement('div');
        loadingMessage.className = 'message system-message info';
        loadingMessage.id = 'loading-messages';
        loadingMessage.innerHTML = `
            <div class="message-content">
                <strong>Sistema:</strong> <span class="loading-text">Carregando mensagens</span>
        ...
        // Adicionar animação nos pontos
        const loadingDots = loadingMessage.querySelector('.loading-dots');
        let dotsCount = 3;
        ...
        // Guardar o interval ID para limpar depois
        loadingMessage.dataset.interval = dotsAnimation;
    }
    ...
    try {
        ...
        // Guarda as mensagens do sistema que não são de carregamento
        const systemMessages = Array.from(chatMessages.querySelectorAll('.system-message:not(#loading-messages)'));
        
        // Escutar mudanças em tempo real
        messagesListener = onSnapshot(q, (snapshot) => {
            // Remover o indicador de carregamento e parar a animação
            const loadingMsg = document.getElementById('loading-messages');
            if (loadingMsg) {
                // Limpar o interval se existir
                if (loadingMsg.dataset.interval) {
                    clearInterval(parseInt(loadingMsg.dataset.interval));
                }
                loadingMsg.remove();
            }
            
            // Se for primeira carga, limpar mensagens antigas
            const noMessages = !document.querySelector('[data-message-id]');
            if (noMessages) {
                chatMessages.innerHTML = '';
                // Restaurar mensagens do sistema
                systemMessages.forEach(msg => chatMessages.appendChild(msg));
            }
            
            // Verifica se existem mensagens no snapshot
            if (snapshot.empty && noMessages) {
                // Se não houver mensagens, apenas mostramos as mensagens do sistema
                console.log('Nenhuma mensagem anterior encontrada para este chat');
                addSystemMessage('Nenhuma mensagem anterior encontrada.', 'info');
                return;
            }
            
            // Processamento otimizado de mudanças
            ...
            snapshot.docChanges().forEach((change) => {
                ...
                if (change.type === "added" || change.type === "modified") {
                    ...
                    // Exibe a mensagem nova ou atualizada
                    displayMessage(data, id);
                }
            });
            
            // Se temos novas respostas, notificar o usuário de forma elegante
            if (hasNewResponse) {
                addSystemMessage(`Nova resposta recebida de Mikael!`, 'success');
                
                // Adicionar um pequeno efeito de destaque nas novas respostas
                const adminResponses = document.querySelectorAll('.admin-response');
                adminResponses.forEach(response => {
                    // Adicionar classe de destaque apenas se não tiver já
                    if (!response.classList.contains('highlight-response')) {
                        response.classList.add('highlight-response');
                        // Remover após 3 segundos
                        setTimeout(() => {
                            response.classList.remove('highlight-response');
                        }, 3000);
                    }
                });
                
                // Tocar um som de notificação se disponível
                try {
                    const notificationSound = new Audio('/assets/sounds/notification.mp3');
                    notificationSound.volume = 0.5;
                    notificationSound.play().catch(e => console.log('Notificação sonora não suportada'));
                } catch (e) {
            ...
            // Tentar conectar silenciosamente após um tempo
            setTimeout(() => {
                if (currentUserName) {
                    // Se ainda tivermos um usuário, tentar reconectar silenciosamente
